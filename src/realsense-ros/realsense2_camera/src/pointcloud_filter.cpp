...
{
    // Must match rs_launch.py keys (see align_depth_filter.cpp).
    static const std::string module_name("pointcloud");

    _params.registerDynamicOptions(*(_filter.get()), module_name);

    std::string param_name = module_name + ".enable";
    _params.getParameters()->setParamT(param_name, _is_enabled, [this](const rclcpp::Parameter& )
        {
            setPublisher();
        });
    _parameters_names.push_back(param_name);

    param_name = module_name + ".allow_no_texture_points";
