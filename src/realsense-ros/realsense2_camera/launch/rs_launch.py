...
D435i defaults in this workspace (edit here, not install/):
  - OpenVINS: infra1/infra2 @ 640x360x30 (OpenVINS track_frequency ~30; was 90 to save USB)
  - Ego-Planner: depth @ 640x360x30 (must match infra 640x360; 640x480 depth won't stream)
  - Color off, align_depth off (saves USB/CPU); pointcloud xyz-only (no RGB texture)
  - enable_gyro/accel=true, initial_reset=false

Rebuild: colcon build --packages-select realsense2_camera
See ../../../../COORDINATES_OPENVINS.md (frames / OpenVINS) and ../../../../JETPACK6_IMU.md (Jetson IMU).
"""
import os
import yaml
...
configurable_parameters = [{'name': 'camera_name',                  'default': 'camera', 'description': 'camera unique name'},
                           ...
                           {'name': 'enable_color',                 'default': 'false', 'description': 'RGB stream (enable for align_depth / textured pointcloud)'},
                           {'name': 'rgb_camera.color_profile',     'default': '640x480x30', 'description': 'color stream profile'},
                           ...
                           {'name': 'depth_module.depth_profile',   'default': '640x360x30', 'description': 'depth stream profile (use 640x360 when infra is 640x360)'},
                           {'name': 'depth_module.depth_format',    'default': 'Z16', 'description': 'depth stream format'},
                           {'name': 'depth_module.infra_profile',   'default': '640x360x30', 'description': 'infra streams (0/1/2) profile'},
                           ...
                           {'name': 'pointcloud.enable',            'default': 'false', 'description': 'publish ~/depth/color/points (off to save USB/CPU)'},
                           {'name': 'pointcloud.stream_filter',     'default': '0', 'description': 'texture: 0=ANY/xyz-only, 1=depth, 2=color, 3=infrared'},
                           {'name': 'pointcloud.stream_index_filter','default': '0', 'description': 'texture stream index for pointcloud'},
                           {'name': 'pointcloud.ordered_pc',        'default': 'false', 'description': ''},
                           {'name': 'pointcloud.allow_no_texture_points', 'default': 'true', 'description': 'full depth FOV in pointcloud without texture stream'},
                           {'name': 'align_depth.enable',           'default': 'false', 'description': 'align depth to color (requires enable_color)'},
